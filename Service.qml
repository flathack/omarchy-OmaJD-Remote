...
Item {
  ...
  readonly property string helperPath: manifest && manifest.__sourceDir
    ? String(manifest.__sourceDir).replace(/\/$/, "") + "/launcher.sh" : ""
  readonly property string installerPath: manifest && manifest.__sourceDir
    ? String(manifest.__sourceDir).replace(/\/$/, "") + "/install.sh" : ""
  ...
  function refresh() { send({ command: "refresh" }) }
  function selectDevice(id) { send({ command: "select_device", device_id: String(id || "") }) }
  function startDownloads() { send({ command: "control", action: "start" }) }
  function stopDownloads() { send({ command: "control", action: "stop" }) }
  function pauseDownloads(value) { send({ command: "control", action: value ? "pause" : "resume" }) }
  function forcePackage(uuid) { send({ command: "force_download", package_ids: [String(uuid)] }) }
  function addLinks(links, autostart) {
    send({ command: "add_links", links: String(links || ""), autostart: autostart === true })
  }
  ...
  function configure(email, password) {
    ...
    send({ command: "configure", email: String(email || ""), password: String(password || "") })
  }
  function forgetAccount() { send({ command: "forget" }) }
  function installHelper() {
    if (installingHelper || installerPath === "") return
  ...
  function handleLine(line) {
    ...
    if (data.type === "fatal") {
      ...
      connected = false
      helperReady = false
      helperMissing = String(data.code || "") === "helper_missing"
  ...
  Process {
    ...
    onExited: function(exitCode) {
      ...
      if (exitCode !== 0 && root.lastError === "") root.lastError = "JDownloader helper exited (" + exitCode + ")"
      if (!root.helperMissing) restartTimer.restart()
    }
  }
  ...
  Process {
    ...
    stdout: SplitParser {
      onRead: function(line) {
        var message = String(line || "").trim()
        if (message !== "") root.actionStatus = message
      }
    }
    stderr: SplitParser {
      onRead: function(line) {
  ...
  Timer {
    ...
    interval: 2500
    repeat: false
    onTriggered: if (root.helperPath !== "" && !daemon.running) daemon.running = true
  }
  ...
  Component.onDestruction: if (daemon.running) daemon.running = false
}
